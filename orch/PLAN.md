...
harness 모노레포의 8개 스킬(ex, re, arch, impl, qa, sec, devops, orch) 전체를 조율하는 **컨트롤 플레인**입니다. 사용자는 두 가지 방식 중 선택할 수 있습니다 — (1) 개별 스킬(ex/re/arch/impl/qa/sec/devops)을 직접 호출하거나, (2) orch를 통해 자연어 요청으로 다중 스킬 파이프라인을 조율합니다. orch는 자연어 요청을 받아 적절한 스킬 또는 사전 정의 파이프라인으로 라우팅하고, 실행(run) 단위로 산출물·상태·로그를 관리합니다.
