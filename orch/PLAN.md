...
harness 모노레포의 9개 스킬(ex, re, arch, impl, qa, sec, devops, verify, orch) 전체를 조율하는 **컨트롤 플레인**입니다. 사용자는 두 가지 방식 중 선택할 수 있습니다 — (1) 개별 스킬(ex/re/arch/impl/qa/sec/devops/verify)을 직접 호출하거나, (2) orch를 통해 자연어 요청으로 다중 스킬 파이프라인을 조율합니다. orch는 자연어 요청을 받아 적절한 스킬 또는 사전 정의 파이프라인으로 라우팅하고, 실행(run) 단위로 산출물·상태·로그를 관리합니다.

orch는 **선택적 오케스트레이션 계층**입니다. 단일 스킬만 필요한 작업은 사용자가 해당 스킬을 직접 호출하는 것이 더 가볍고 직접적이며, 다중 스킬 협업·파이프라인·재개 가능한 장기 실행이 필요할 때 orch를 사용합니다.
...
| 사용자 진입점 | API/CLI 다수 | **이중 진입점 — 개별 스킬 직접 호출 또는 orch를 통한 파이프라인 실행** |
...
orch는 모노레포의 8개 콘텐츠 스킬을 레지스트리로 인식합니다. 각 스킬은 `<skill>/SKILL.md`라는 표준 진입점과 산출물 디렉토리(`runs/<id>/<skill>/`)를 가집니다.
...
| verify | ��컬 통합 검증 | 3섹션 (환경 설정, 검증 시나리오, 검증 리포트) |
...
- 모든 메타데이터 조작은 orch의 `${SKILL_DIR}/scripts/artifact.py`를 통해서만 이루어지며, 콘텐츠 스킬은 본문(`*.md`)만 편집합니다.
...
orch 에이전트(스테이지)는 **YAML을 직접 편집하지 않습니다**. 모든 상태 전이는 `${SKILL_DIR}/scripts/run.py`(또는 `artifact.py`)를 거치며, 이는 스키마 검증·타임스탬프 자동화·승인 이력 append-only를 보장합니다.
...
orch (dispatch → pipeline)
    ...
    ▼
콘텐츠 스킬(ex/re/arch/impl/qa/sec/devops/verify)
```
...
└── references/
    ...
    ├── pipelines/                        # 사전 정의 파이프라인 (10개, orch 고유)
    │   ├── full-sdlc.md
    ...
    │   └── integration-verify-existing.md
    ├── contracts/
...
- **역할**: orch 사용 시의 진입점. 자연어 요청을 분석하여 스킬 또는 파이프라인으로 라우팅 (사용자는 orch를 거치지 않고 개별 스킬을 직접 호출하는 경로도 선택 가능)
- **핵심 역량**: 자연어 의도 분석, 복합 요청의 다중 스킬 분해, 선행 조건 검증, 기존 프로젝트 감지(ex 선행 배치), 재개 요청 인식, `current-run.md` 단일 파일로 즉시 컨텍스트 파악
- **입력**: 현재 대화 턴의 사용자 요청 + 필요 시 직접 읽은 `current-run.md` 스냅샷
- **출력**: 파이프라인 정의(스킬 목록 + 순서 + 병렬 그룹) 또는 단일 스킬 호출 지시

### `references/workflow/pipeline.md`
- **역할**: DAG 기반 워크플로 실행, 스킬 간 흐름 제어
- **핵심 역량**: 순차/병렬/조건부 실행, 각 스킬 실행 시 `HARNESS_ARTIFACTS_DIR=<output-root>/runs/<run_id>/<skill>` 및 `HARNESS_RUN_ID=<run_id>` 환경변수 주입, `needs_user_input` 감지 → relay 위임, 체크포인트 기록, 안전한 격리가 가능할 때만 병렬 실행, 완료 후 보고서 생성
...
- **핵심 역량**: `run.py init-run`으로 디렉토리·메타데이터 초기화, 산출물 골격 생성, 필수 섹션 검증(EX=4, RE=3, ARCH=4, IMPL=4, QA=4, SEC=4, DEVOPS=4, VERIFY=3), `run.py update-state`로 단계 상태 반영, `run.py render`로 `run.meta.md` / `current-run.md` 동기화, 완료 시 `current-run.md`를 idle로 갱신
- **입력**: 파이프라인 정의, 출력 설정
...
orch SKILL.md는 표준 frontmatter를 가지지만, 실행 시에는 모델이 직접 `current-run.md`를 읽고 현재 대화 턴의 사용자 요청을 입력으로 사용합니다.

```markdown
1. Read `harness-output/current-run.md` if it exists
2. Treat the current conversation turn as the user request
3. Load only the workflow reference needed for the current stage
```

- **Current State**: 활성 run 스냅샷은 필요 시 파일에서 직접 읽음
- **Skill Root**: 내부 참조는 상대 경로 기준으로 해결
- **User Request**: 현재 대화 턴의 사용자 요청을 그대로 사용

이 패턴은 다른 스킬에는 없는 orch 고유의 설계로, **다중 스킬 파이프라인의 조율자**라는 책임에서 비롯됩니다. 사용자가 개별 스킬을 직접 호출하는 경로에서는 이 추가 상태 읽기가 필요하지 않습니다.
...
대화형 스킬(re:elicit, re:analyze, re:spec, re:review, arch:design, sec:threat-model)은 의미 단위마다 `needs_user_input` 사용. 자동 실행 스킬(ex/impl/qa/sec:audit/devops/verify)은 예외 조건에서만 사용.
...
orch가 즉시 라우팅 가능한 10개의 사전 정의 파이프라인. 각각 독립 파일로 정의되어 pipeline 스테이지가 참조합니다.

| 파이프라인 | 흐름 |
|-----------|------|
| `full-sdlc` | re:elicit → re:analyze → re:spec → arch:design → impl:generate → [qa:generate, sec:audit, devops:pipeline] → verify:provision → verify:execute |
| `full-sdlc-existing` | ex(4단계) → re(3단계) → arch:design → impl:generate → [qa, sec, devops] → verify:provision → verify:execute |
...
| `integration-verify` | verify:provision → verify:instrument → verify:scenario → verify:execute → verify:diagnose → verify:report |
| `integration-verify-existing` | ex(4단계) → verify:provision → verify:instrument → verify:scenario → verify:execute → verify:diagnose → verify:report |

병렬 그룹(`[qa:generate, sec:audit, devops:pipeline]`)은 논리적 병렬 단위입니다. 런타임이 안전한 격리를 지원할 때만 동시 실행하고, 그렇지 않으면 `impl` 이후 순차 실행합니다.
...
3. 스크립트 호출 규약 (`${SKILL_DIR}/scripts/run.py`)
...
- `references/pipelines/` 10개 (full-sdlc, full-sdlc-existing, new-feature, new-feature-existing, security-gate, security-gate-existing, quick-review, explore, integration-verify, integration-verify-existing)
...
1. **행동 규약 (가장 약함)**: `references/workflow/*.md`와 SKILL.md 본문에서 "메타데이터 갱신은 반드시 `${SKILL_DIR}/scripts/run.py`를 호출한다"를 반복 명시
...
1. **이중 진입점 (Dual Entry Modes)**: 사용자는 두 가지 방식 중 선택할 수 있다 — (a) 개별 콘텐츠 스킬을 직접 호출하는 경량 경로, (b) orch를 통해 다중 스킬 파이프라인을 조율하는 중량 경로. orch는 강제되는 게이트가 아니라 다중 스킬 협업·재개·추적성이 필요할 때 선택하는 오케스트레이션 계층이다.
2. **스킬 브로커 (Skill Broker)**: 자연어 요청을 분석하여 스킬/파이프라인으로 라우팅. orch는 콘텐츠를 생산하지 않고 콘텐츠 스킬 사이의 입출력을 중개.
3. **명시적 상태 로딩 (Explicit State Loading)**: orch는 필요 시 `current-run.md`를 직접 읽고 현재 대화 턴의 사용자 요청을 입력으로 사용한다.
...
7. **메타데이터-문서 분리 및 스크립트 경유 원칙**: run 상태는 `run.meta.yaml`(상태/승인/추적성)과 `run.meta.md`/`current-run.md`(사람 읽기용 렌더링)로 분리. 메타데이터는 에이전트가 직접 편집하지 않고 오직 `${SKILL_DIR}/scripts/run.py` 커맨드를 통해서만 갱신. 사람 읽기용 문서는 `assets/templates/`의 사전 정의 템플릿으로 골격을 생성한 뒤 스크립트 렌더링으로 채워, 상태 일관성과 서식 표준을 동시에 보장.
8. **Codex Skill 표준 준수 (Standard Compliance)**: 단일 진입점 `SKILL.md` + YAML frontmatter, `scripts/`·`assets/`·`references/`의 표준 디렉토리 명칭, 상대 경로 기반 참조, 명시적 상태 로딩을 통해 현재 Codex 런타임과 정합성을 유지.
