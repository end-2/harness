...
- **Skill Root**: `${SKILL_DIR}`을 모든 내부 참조 경로의 기준점으로 사용 (하드코딩 방지)
- **User Request**: 사용자 자연어 요청을 표준 `$ARGUMENTS` 치환자로 수신

이 패턴은 다른 스킬에는 없는 orch 고유의 설계로, **다중 스킬 파이프라인의 조율자**라는 책임에서 비롯됩니다. 사용자가 개별 스킬을 직접 호출하는 경로에서는 이 동적 컨텍스트 주입이 필요하지 않습니다.
...
3. **동적 컨텍스트 주입 (Dynamic Context Injection)**: SKILL.md 상단에서 `current-run.md` 스냅샷, `${SKILL_DIR}`, `$ARGUMENTS`를 harness가 매 호출마다 주입. 모델이 아닌 harness가 컨텍스트 로딩을 책임.
...
8. **Claude Code Skill 표준 준수 (Standard Compliance)**: 단일 진입점 `SKILL.md` + YAML frontmatter, `scripts/`·`assets/`·`references/`의 표준 디렉토리 명칭, `${SKILL_DIR}`/`$ARGUMENTS` 치환자 활용, 동적 컨텍스트 주입(`` !`<command>` ``)을 통해 공식 표준(https://code.claude.com/docs/ko/skills)과 완전히 호환.
