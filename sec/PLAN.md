...
에이전트는 **YAML 메타데이터 파일을 직접 편집하지 않습니다**. 모든 상태 갱신은 `${SKILL_DIR}/scripts/` 디렉토리의 스크립트 커맨드를 통해서만 수행하며, 이는 다음을 보장합니다.
...
- 감사 증적의 원자적 기록 (`approval.approver`, `approval.rationale`, `${SESSION_ID}`)

핵심 스크립트:

| 스크립트 커맨드 | 용도 |
|----------------|------|
| `${SKILL_DIR}/scripts/artifact.py init --section <name> --id <id>` | 메타데이터 + markdown 템플릿 쌍을 새로 생성 |
| `${SKILL_DIR}/scripts/artifact.py set-phase <id> <phase>` | 진행 단계 전이 (전이 규칙 검증 포함) |
| `${SKILL_DIR}/scripts/artifact.py set-progress <id> --completed N --total M` | 진행률 갱신 |
| `${SKILL_DIR}/scripts/artifact.py link <id> --upstream <ref>` / `--downstream <ref>` | 추적성 ref 추가 (양방향 무결성 유지) |
| `${SKILL_DIR}/scripts/artifact.py show <id>` | 메타데이터 + 문서 합쳐 조회 |
| `${SKILL_DIR}/scripts/approval.py request <id>` | 승인 요청 상태로 전환 |
| `${SKILL_DIR}/scripts/approval.py approve <id> --approver <name> --rationale ...` | 승인 처리 및 감사 증적 기록 |
| `${SKILL_DIR}/scripts/approval.py reject <id> --approver <name> --rationale ...` | 반려 처리 |
| `${SKILL_DIR}/scripts/approval.py accept-risk <id> --approver <name> --rationale ...` | 리스크 수용 기록 (감사 증적의 핵심 케이스) |
| `${SKILL_DIR}/scripts/validate.py [<id>]` | 스키마/추적성 검증 |
| `${SKILL_DIR}/scripts/report.py summary` | 4섹션 메타데이터 종합 상태 리포트 (SKILL.md 동적 컨텍스트 주입용) |
...
| `sec-record` (쓰기 전용 보조 스킬) | 메타데이터/승인 상태/감사 증적 기록 | `Read` + `Bash(python3 ${SKILL_DIR}/../sec/scripts/artifact.py:*)` `Bash(python3 .../approval.py:*)` `Bash(python3 .../validate.py:*)` `Bash(python3 .../report.py:*)` | **`disable-model-invocation: true`** — 사용자 명시 호출만 가능 |
...
- 두 스킬은 동일한 `sec/scripts/` 디렉토리를 공유합니다(`sec-record`는 `${SKILL_DIR}/../sec/scripts/`로 참조).
...
4. 4단계 워크플로우 요약 (threat-model → audit → review → compliance)
   ...
   - 예: "감사 시 `${SKILL_DIR}/references/workflow/audit.md`를 Read로 로드한 뒤 지시를 따른다"
5. 스크립트 호출 규약: 모든 메타데이터 조작은 `${SKILL_DIR}/scripts/artifact.py`·`approval.py`·`validate.py`·`report.py`를 통해서만 수행. 메인 `sec`는 직접 호출 권한이 없으므로 사용자에게 `sec-record` 호출을 명시적으로 안내
6. 시작 시 현재 상태 주입: SKILL.md 상단에서 `` !`python3 ${SKILL_DIR}/scripts/report.py summary` ``를 사용해 현재 4섹션 산출물 상태·승인·갭 요약을 동적 컨텍스트로 주입
...
- 모든 스크립트 경로는 `${SKILL_DIR}/scripts/...`로 작성하여 사용자 호출 위치에 관계없이 동작하도록 합니다.
- `$ARGUMENTS`는 `--mode lightweight|heavy`, `--standard owasp-asvs-l2|pci-dss|gdpr|hipaa` 같은 인자를 받는 데 사용합니다.
- `${SESSION_ID}`는 감사 증적 기록 시 세션 식별자로 `approval.py`에 전달합니다.
...
- **리스크 수용/critical 에스컬레이션은 감사 증적**: `mitigation_status: accepted` 처리나 critical 취약점에 대한 사용자 결정은 반드시 `approval.py accept-risk` 또는 `approval.py approve --rationale ...`로 기록되도록 안내. 감사 증적에는 `${SESSION_ID}`와 `approval.approver`, `approval.rationale`이 필수로 포함
...
- `scripts/artifact.py`: `init` / `set-phase` / `set-progress` / `link` / `show` 서브커맨드. `init`는 `${SKILL_DIR}/assets/templates/`에서 템플릿을 복사해 메타데이터 + markdown 쌍을 생성. 모든 쓰기 커맨드는 `updated_at`을 자동 갱신하고 잘못된 상태 전이를 차단
- `scripts/approval.py`: `request` / `approve` / `reject` / `accept-risk` 서브커맨드. 각 커맨드는 `approval.approver`, `approval.approved_at`, `approval.rationale`, `${SESSION_ID}`를 메타데이터에 원자적으로 기록하여 **감사 증적**을 남김. `approve`/`reject` 직행 같은 잘못된 전이는 차단
...
1. **행동 규약 (가장 약함)**: `references/workflow/*.md` 및 SKILL.md 본문에서 "메타데이터 갱신은 반드시 `${SKILL_DIR}/scripts/`의 커맨드를 호출한다"를 반복 명시
...
4. **시작 시 상태 주입 (최강)**: SKILL.md 상단에서 `` !`python3 ${SKILL_DIR}/scripts/report.py summary` ``를 사용해 현재 산출물 상태·추적성 무결성·감사 증적을 동적으로 주입. 이를 통해 에이전트가 자연스럽게 스크립트 기반 흐름을 따르도록 유도
