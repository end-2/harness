...
- `description`은 250자 이내에서 "무엇(what) + 언제(when)"을 front-load합니다.
- `name`은 lowercase/digits/hyphens 규칙을 따라 `sec`로 고정하고 디렉토리도 `sec/`와 일치시킵니다.
- Claude Code Skill 표준은 `name`/`description`만 필수로 요구하므로, 그 외 옵션 필드(`allowed-tools`, `disable-model-invocation`, `model`, `effort`, `argument-hint` 등)는 기본 동작으로 목적을 달성할 수 없는 구체적 사유가 있을 때에만 추가합니다. 본 스킬에서 정당화되는 유일한 예외는 audit 단계 sub-agent의 `context: fork`이며, 대용량 코드 감사를 메인 컨텍스트에서 격리해야 한다는 구체적 사유가 있습니다.
