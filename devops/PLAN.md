...
에이전트는 **YAML 메타데이터 파일을 직접 편집하지 않습니다**. 모든 상태 갱신은 `${SKILL_DIR}/scripts/artifact.py`를 통해서만 수행하며, 이는 (1) 스키마 검증, (2) `updated_at` 자동 갱신, (3) 추적성 ref 양방향 무결성, (4) 승인 상태 전이 규칙(`draft → approved` 직행 금지 등)을 보장합니다.

핵심 스크립트 커맨드:

| 커맨드 | 용도 |
|--------|------|
| `${SKILL_DIR}/scripts/artifact.py init <kind> <id>` | 메타데이터 + markdown 템플릿 쌍 생성 |
| `${SKILL_DIR}/scripts/artifact.py set <id> --field k=v` | 도메인 필드 값 갱신 |
| `${SKILL_DIR}/scripts/artifact.py transition <id> --to <phase>` | 진행 단계 전이 (전이 그래프 검증) |
| `${SKILL_DIR}/scripts/artifact.py approve <id> --approver <name>` | 승인 상태 전이 |
| `${SKILL_DIR}/scripts/artifact.py link <id> --upstream <ref>` / `--downstream <ref>` | 추적성 ref 추가 |
| `${SKILL_DIR}/scripts/artifact.py show <id>` | 메타데이터 조회 |
| `${SKILL_DIR}/scripts/artifact.py validate [<id>]` | 스키마/추적성 검증 |
| `${SKILL_DIR}/scripts/artifact.py list [--kind ...] [--phase ...]` | 메타데이터 인덱스 조회 (review 단계용) |
...
3. 적응적 깊이 분기 로직
4. 8단계 워크플로우 요약 (각 단계 진입 시 `${SKILL_DIR}/references/workflow/<stage>.md`를 Read로 로드)
5. 스크립트 호출 규약: 모든 메타데이터 조작은 `${SKILL_DIR}/scripts/artifact.py`만 사용
6. 시작 시 상태 주입: `` !`python ${SKILL_DIR}/scripts/artifact.py validate` ``로 현재 산출물 상태를 동적 컨텍스트로 주입
7. 선행 산출물 존재 확인: `` !`ls arch/artifacts` ``, `` !`ls impl/artifacts` `` 동적 주입

**치환자**: 모든 스크립트/참조 경로는 `${SKILL_DIR}/...` 로 작성, 사용자 인자는 `$ARGUMENTS`로 받음.
...
스크립트 호출 규약("메타데이터 갱신은 반드시 `${SKILL_DIR}/scripts/artifact.py` 호출")은 각 워크플로우 파일에 반복 명시하고, 각 단계에서 어떤 커맨드를 어떤 순서로 호출해야 하는지 시퀀스로 기술합니다.
...
1. **행동 규약 (가장 약함)**: `references/workflow/*.md`와 SKILL.md 본문에서 "메타데이터 갱신은 반드시 `${SKILL_DIR}/scripts/artifact.py` 호출"을 반복 명시
...
4. **시작 시 상태 주입**: SKILL.md 상단에서 `` !`python ${SKILL_DIR}/scripts/artifact.py validate` ``로 현재 산출물 상태·추적성 무결성을 동적 주입하여 에이전트가 자연스럽게 스크립트 흐름을 따르도록 유도
...
7. **메타데이터-문서 분리 및 스크립트 경유 원칙 (Metadata/Document Separation via Scripts)**: 산출물은 YAML 메타데이터 파일과 markdown 문서 파일을 분리하여 관리하고, 메타데이터(진행 상태·승인 상태·추적성 ref)는 에이전트가 직접 편집하지 않고 오직 `${SKILL_DIR}/scripts/artifact.py` 커맨드를 통해서만 갱신. markdown 본문은 `assets/templates/`의 사전 정의 템플릿으로 골격을 생성한 뒤 에이전트가 플레이스홀더를 채움으로써 상태 일관성과 서식 표준을 동시에 보장. 실행 가능한 IaC/파이프라인 YAML과 상태 관리용 메타데이터 YAML(`.meta.yaml`)은 디렉토리·확장자로 엄격히 구분
