#!/usr/bin/env python3
"""Artifact manager for the DevOps skill.

Single entry point for all metadata manipulation. The DevOps skill must not edit
...
def artifacts_dir() -> Path:
    ...
    if env:
        ...
        base = Path.cwd() / "artifacts" / "devops"
...
def cmd_list(_args: argparse.Namespace) -> int:
    ...
    for aid, section, phase, approval in rows:
        print(f"{aid:<{width}}  {section:<16}  {phase:<10}  {approval}")
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    ...
    rp.add_argument(
        "--stage",
        required=True,
        help="workflow stage producing the report (e.g. monitor, review)",
    )
    rp.add_argument(
        "--target",
        action="append",
