...
argument-hint: "[project-root] [--depth lite|heavy] [--budget N] [--focus <path>] [--out <dir>]"
...
- `argument-hint`: 필수 1개(`project-root`) + 선택 플래그 4개 노출. 사용자가 스킬 호출 시 자동완성에 노출됨.
- `allowed-tools`: `Read Glob Grep` (코드 스캔) + `Bash`(`scripts/artifact.py` 실행). **`Write`는 의도적으로 제외**하여 비파괴 원칙을 기술적으로 강제.
- `context: fork`: 대량 파일 스캔으로 인한 메인 컨텍스트 오염을 차단. Phase 1~3 결과는 fork된 서브컨텍스트에서 소비되고, Phase 4의 최종 산출물만 메인 컨텍스트로 반환.
- `agent: Explore`: 서브에이전트로 Explore 활용. 본 스킬의 토큰 예산 제어(기본 4000) 원칙과 정확히 일치.
- `disable-model-invocation`: 생략(= false). 자동 호출 허용. 보안 감사 변형이 필요할 경우 주석으로 `true` 전환 안내를 SKILL.md 하단에 남김.
- `user-invocable`: 생략(= true). 사용자가 직접 호출 가능.
