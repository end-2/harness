...
모든 스킬(re, arch, impl, qa, sec, devops)의 실행을 조율하는 컨트롤 플레인입니다. 사용자는 항상 orchestration을 통해 통신하며, 개별 스킬을 직접 호출하지 않습니다.

| Agent | 설명 |
|-------|------|
| `orchestration:dispatch` | 사용자의 유일한 진입점. 자연어 요청을 분석하여 스킬 또는 파이프라인으로 라우팅 |
| `orchestration:pipeline` | DAG 기반 워크플로 실행, 스킬 간 흐름 제어 (순차/병렬/조건부 분기) |
| `orchestration:relay` | 실행 중인 스킬 에이전트와 사용자 간의 멀티턴 대화 중계 |
| `orchestration:run` | 실행(run) 생명주기 관리, 산출물 디렉토리 생성/관리, 상태 추적, 재개 지원 |
| `orchestration:config` | 스킬 설정, 에이전트 규칙, 파이프라인 템플릿 관리 |
| `orchestration:status` | 실행 이력 조회, 스킬 상태 확인, 산출물 검색 |
