...
모든 에이전트는 산출물을 **메타데이터 파일(`meta.json`)** 과 **본문 파일
(`body.md`)** 의 쌍으로 생성합니다. 구조화 데이터(진행 상태, 승인 결과,
RE 참조, 스킬별 페이로드)는 `meta.json`에, 서술·근거·다이어그램 등
사람이 읽는 맥락은 `body.md`에 기록됩니다.

```
runs/<run_id>/<skill>/<agent>[-NN]/
├── meta.json   # 상태 · 승인 · refs · data
└── body.md     # 서술 · 근거 · 다이어그램
```

`meta.json`은 반드시 `scripts/artifact` CLI를 통해서만 조작합니다.
`body.md`는 템플릿(`templates/<skill>/<agent>.md`)으로부터 기본 골격이
자동 생성되며, 에이전트가 직접 편집합니다.

```bash
# 산출물 쌍 생성 (run_id/artifact_id를 출력)
./scripts/artifact init --skill arch --agent design --title "Checkout redesign"

# 본문 경로 획득 후 편집
./scripts/artifact path arch-design-01 --run-id <id> --body

# 구조화 데이터 병합 및 진행 상태 갱신
./scripts/artifact set arch-design-01 --run-id <id> \
    --progress review --data-file patch.json --ref-re FR-001

# 리뷰 에이전트의 최종 판정
./scripts/artifact set arch-review-01 --run-id <id> \
    --verdict APPROVED --approver lead --notes "meets NFRs"

# run 전체 롤업
./scripts/artifact list --run-id <id>
./scripts/artifact validate arch-design-01 --run-id <id>
```

상세 스키마와 템플릿 규약은 [`templates/README.md`](templates/README.md)를
참고하세요.
